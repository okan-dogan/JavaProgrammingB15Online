...
public class Task_JobTitles {
    ...
    public static void main(String[] args) {
        ...
        for (String eachJob : employees) {
            System.out.println(eachJob.substring(eachJob.indexOf("[")+1,eachJob.indexOf("]")));
        }
        ...
        for (int i=0;i<employees.length;i++) {
            replaceW=employees[i].replace(" [","[");
            System.out.println(replaceW.substring(0,replaceW.indexOf(" "))+"-"
                    +replaceW.substring(replaceW.lastIndexOf(" ",replaceW.indexOf("["))+1,replaceW.indexOf("["))
                    +" "+replaceW.substring(replaceW.indexOf("[")+1,replaceW.indexOf("]")));
            if(replaceW.contains("Tracy Bartle")&&replaceW.contains("Geological Engineer")) {
                counter1++;

            }if(replaceW.contains("Skipper Abrahamsson")&&employees[i].contains("Financial Advisor")){
                counter2++;
            }
        }
        if(counter1==1){
        ...
        for (int i=0;i<employees.length;i++) {
            replaceW = employees[i].replace(" [", "[");
            System.out.println(replaceW.substring(0, 1) + "."
                    + replaceW.substring(replaceW.lastIndexOf(" ", replaceW.indexOf("[")) + 1, replaceW.indexOf("[")).substring(0,1)
                    + "." +"==>" +replaceW.substring(replaceW.indexOf("[") + 1, replaceW.indexOf("]")));
        }
